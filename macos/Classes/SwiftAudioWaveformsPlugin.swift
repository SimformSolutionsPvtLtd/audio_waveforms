...
public class SwiftAudioWaveformsPlugin: NSObject, FlutterPlugin {
    ...
    /// Initializes the plugin with Flutter registrar and method channel.
    /// - Parameters:
    ///   - registrar: Flutter plugin registrar
    ///   - flutterChannel: Method channel for Flutter communication
    init(
    ...
    /// - Parameter registrar: The plugin registrar
    ...
    /// Routes method calls to appropriate handlers for recording, playback, and extraction.
    /// - Parameters:
    ///   - call: The method call from Flutter
    ///   - result: Callback to return the result
    public func handle(
        ...
        case Constants.startRecording:
            ...
            audioRecorder.startRecording(
                result, RecordingSettings.fromJson((args)))
            break
        ...
        case Constants.startPlayer:
            let key = args?[Constants.playerKey] as? String
            if key != nil {
                audioPlayers[key!]?.startPlayer(result: result)
            } else {
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
        ...
        case Constants.seekTo:
            ...
            if key != nil {
                ...
                result(
                    FlutterError(
                        code: Constants.audioWaveforms,
                        message: "Can not seek to position",
                        details: "Player key is null"))
            }
    ...
    /// Initializes a new audio player instance if not already created.
    /// - Parameter playerKey: Unique identifier for the player
    ...
    /// Creates or updates a waveform extractor for the specified audio file.
    /// - Parameters:
    ///   - playerKey: Unique identifier for the extractor
    ///   - result: Callback with extracted waveform data
    ///   - path: File path of the audio file
    ///   - noOfSamples: Number of samples to extract
