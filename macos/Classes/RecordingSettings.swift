...
//  audio_waveforms
//
//  Created by Vasu Nageshri on 02/01/26.
//
...
struct RecordingSettings {
    ...
    static func fromJson(_ json: [String: Any]) -> RecordingSettings {
        ...
        let overrideAudioSession =
            json[Constants.overrideAudioSession] as? Bool ?? true
        let linearPCMBitDepth = json[Constants.linearPCMBitDepth] as? Int ?? 16
        let linearPCMIsBigEndian =
            json[Constants.linearPCMIsBigEndian] as? Bool ?? false
        let linearPCMIsFloat =
            json[Constants.linearPCMIsFloat] as? Bool ?? false
